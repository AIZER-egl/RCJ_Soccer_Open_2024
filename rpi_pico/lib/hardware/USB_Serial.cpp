 ...
 *  Author Chris
 *
 *  This is not aizer code, make sure to star the original repo :)
 *  https://github.com/MrChros/RP2040-Library/blob/main/USB%20Serial/USB_Serial.h
 */
...
bool USB_Serial_Send_Int_Dec(int value, unsigned int numbers)
{
    if(value < 0) {
        USB_Serial_Put_Char('-');
        value = value * -1;
...
bool USB_Serial_Send_Int_Hex(unsigned int value, unsigned int numbers, bool Send_0x)
{
    if(Send_0x == true)
    {
...
bool USB_Serial_Send_Float(float value, unsigned int numbers, unsigned int decimals, bool force_sign)
{
